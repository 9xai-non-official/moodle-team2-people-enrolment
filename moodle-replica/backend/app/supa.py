"""
Supabase (PostgREST) access layer (Mahdi's — was app/db.py before the merge;
renamed because app/db.py is the asyncpg pool. Team follow-up: pick ONE layer).
...
def _client() -> httpx.Client:
    # Lazy check (was import-time) so the app still boots without env and
    # answers a clear 503 instead of crashing uvicorn.
    if not SUPABASE_URL or not SUPABASE_SERVICE_KEY:
        from fastapi import HTTPException

        raise HTTPException(
            status_code=503,
            detail="SUPABASE_URL / SUPABASE_SERVICE_KEY missing — see "
            "backend/.env.example and TEAM_CREDENTIALS.md",
        )
